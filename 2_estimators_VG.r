...
Mstep_pibeta = function(n=c(100,100,100),K = 3,q = 4,p = 10,nx=4,
                 x,y,z,C,alphai,alphai2,alphai22, alphai2Q,tau,cste=cste){
  ...
  return(list(piik=piik,beta = beta))
}

Mstep_thetamu = function(n=c(100,100,100),K = 3,q = 4,p = 10,nx=4,
                 x,y,z,C,old.beta,old.Q,alphai,alphai2,alphai22, alphai2Q,tau,cste=cste){
  ...
  for (k in 1:K){
    ...
    alphai22k = alphai22[[k]]
    alphai2Qk = alphai2Q[[k]]
    old.betak=old.beta[[k]]
    old.Qk=old.Q[[k]]
    #tau  = as.numeric(z==k)
    
    #betak = matrix(as.numeric(beta[[k]]),q,q)
    for (i in (1:nn)){
      theta2i[i,k] =  tauik[i]*(alphai2k[i]-2*t(X[i,])%*%t(old.betak)%*%alphaik[,i]+t(X[i,])%*%t(old.betak)%*%old.betak%*%X[i,])
    }
    theta2 = (1/(nn*q))*sum(theta2i)
    

    mu[[k]] = apply(rep(1,p)%*%t(tauik) * (t(y)-old.Qk%*%alphaik),1,sum)/sum(tauik)
    #veci = numeric(nn)
    #for (i in 1:nn){ veci[i] = t(t(y[i,]-mu[[k]]))%*%Q[[k]]%*%(alphaik[,i])}
  }
  return(list(mu=mu,theta2=theta2))
}

Mstep_Qsigma = function(n=c(100,100,100),K = 3,q = 4,p = 10,nx=4,
  ...
  for (k in 1:K){
    ...
    #veci = numeric(nn)
    #for (i in 1:nn){ veci[i] = t(t(y[i,]-mu[[k]]))%*%Q[[k]]%*%(alphaik[,i])}
    sigma2i[k] = sum(tauik * (diag(t(t(y)-old.muk)%*%(t(y)-old.muk))-2*diag(t(t(y)-old.muk)%*%Q[[k]]%*%alphaik)+alphai2Q[[k]]))
  }
  sigma2 = (1/(nn*p))*sum(sigma2i)
  sigma2i = sigma2i / nn
  return(list(Q=Q,sigma2=sigma2,sigma2i=sigma2i))
}
estimates = function(data,K=3, par_init = NULL, maxits=100,
  ...
  while (diff > tol && iter < maxits){
    ...
    Mstepresults = Mstep_pibeta(n,K,q,p,nx,x,y,z,C,alphai,alphai2,alphai22,alphai2Q,tau,cste)
    piik=Mstepresults$piik;beta=Mstepresults$beta
    
    Estepresults = Estep(n,K,q,p,nx,piik,mu,beta,theta2,sigma2,x,y,Q,C,cste)
    alphai=Estepresults$alphai;alphai2=Estepresults$alphai2;alphai22=Estepresults$alphai22;alphai2Q=Estepresults$alphai2Q;tau=Estepresults$tau
    
    Mstepresults = Mstep_thetamu(n,K,q,p,nx,x,y,z,C,old.beta,old.Q,alphai,alphai2,alphai22,alphai2Q,tau,cste)
    mu=Mstepresults$mu;theta2=Mstepresults$theta2
    
    Estepresults = Estep(n,K,q,p,nx,piik,mu,beta,theta2,sigma2,x,y,Q,C,cste)
    alphai=Estepresults$alphai;alphai2=Estepresults$alphai2;alphai22=Estepresults$alphai22;alphai2Q=Estepresults$alphai2Q;tau=Estepresults$tau
    
    Mstepresults = Mstep_Qsigma(n,K,q,p,nx,x,y,z,C,old.mu,alphai,alphai2,alphai22,alphai2Q,tau,cste)
    Q=Mstepresults$Q;sigma2=Mstepresults$sigma2;sigma2i=Mstepresults$sigma2i
